...
#directory_name = "./85_failed_jobs/"
#csv_filename = "RHEL85_build_status.csv"
#csv_header = ["Job name", "Job status", "Owner", "New Status", "Comments"]
...
def get_85_failed_jobs(server):
    rhel85_failed_jobs_list = []
    view = config_object["JENKINS_VIEW"]
    # rhel83_jobs is list of dictionaries
    rhel85_jobs_list = server.get_jobs(view_name=view["RHEL85_VIEW"])
    for job in rhel85_jobs_list:
        # checking jobs with status as failed 
        if job['color'] == "red":
           rhel85_failed_jobs_list.append(job)
    #print("rhel83_failed_jobs_list >>>> ", rhel83_failed_jobs_list)
    print()
    print("Total Failed jobs : ", len(rhel85_failed_jobs_list))
    print()
    # returning list of failed jobs
    return rhel85_failed_jobs_list

def get_build_script(server, rhel85_failed_jobs_list):
    # job_build_script dictionary contains job_name as key & value as script
    job_build_script = {}
    # creating  directory 
    if not os.path.exists(output_data["RHEL85_OUTPUT_DIR_NAME"]):
        os.mkdir(output_data["RHEL85_OUTPUT_DIR_NAME"])
        print("Directory '%s' created."%(output_data["RHEL85_OUTPUT_DIR_NAME"]))
        print()
    else:
        print("Directory '%s' already exists."%(output_data["RHEL85_OUTPUT_DIR_NAME"]))
        print()
    for job in rhel85_failed_jobs_list:
        job_name = job['name']
...
def store_data(server, job_build_script):
    for job in job_build_script:
        with open(os.path.join(output_data["RHEL85_OUTPUT_DIR_NAME"], job + '.sh'), 'w') as file_name:
            file_name.write(job_build_script[job])
    shutil.make_archive(output_data["RHEL85_OUTPUT_DIR_NAME"], 'zip', output_data["RHEL85_OUTPUT_DIR_NAME"])
    return "Complete"
...
def generate_report(server):
    job_status_dict = {}
    view = config_object["JENKINS_VIEW"]
    rhel85_jobs = server.get_jobs(view_name=view["RHEL85_VIEW"])
    for job in rhel85_jobs:
        job_name = job['name']
        job_status = job['color']
        if job_status == "red":
            job_status_dict[job_name] = job_status
    #with open('mycsvfile.csv', 'w') as f:
    with open(output_data["RHEL85_CSV_FILE_NAME"], 'w') as f:
        w = csv.writer(f)
...
def main():
    ...
    rhel85_failed_jobs_list = get_85_failed_jobs(server)
    job_build_script = get_build_script(server, rhel85_failed_jobs_list)
